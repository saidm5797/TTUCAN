...
IsoTp::IsoTp(INT8U _CS, uint8_t mcp_int) : MCP_CAN(_CS)
{
  _mcp_int = mcp_int;
}

uint8_t IsoTp::isoBegin(INT8U idmodeset, INT8U speedset, INT8U clockset)
{
    begin(MCP_ANY, CAN_500KBPS, MCP_8MHZ);
}

uint8_t IsoTp::isoSetMode(const INT8U opMode)
{
    setMode(opMode);
}
...
#endif
  return sendMsgBuf(id, 0, len, data);
}
...
uint8_t IsoTp::can_receive(void)
{
  ...
  if (_mcp_int)
    ...
    msgReceived = (checkReceive() == CAN_MSGAVAIL);       // No IRQ: poll receive buffer
  
  if (msgReceived)
  {
     memset(rxBuffer,0,sizeof(rxBuffer));       // Cleanup Buffer
     readMsgBuf(&rxId, &rxLen, rxBuffer); // Read data: buf = data byte(s)
#ifdef ISO_TP_DEBUG
