...
class IsoTp : protected MCP_CAN
{
	public:
		IsoTp(INT8U _CS, uint8_t mcp_int);
		uint8_t isoBegin(INT8U idmodeset, INT8U speedset, INT8U clockset);
		uint8_t isoSetMode(INT8U opMode);
		uint8_t send(Message_t* msg);
		uint8_t receive(Message_t* msg);
		void    print_buffer(uint32_t id, uint8_t *buffer, uint16_t len);
	private:
        uint8_t  _mcp_int;
		uint32_t rxId; //protect?
		uint8_t  rxLen;
		uint8_t  rxBuffer[8];
    uint16_t rest;
